...
Wir sehen, dass die Gentoo-Pinguine mit einem durchschnittlichen Körpergewicht von etwa `r round(CIs$response[CIs$species == "Gentoo"], 0)` g signifikant schwerer sind als Adelie (p \< `r format_p(CIp$p.value[CIp$contrast == "Gentoo - Adelie"])`) und Chinstrap (p \< `r format_p(CIp$p.value[CIp$contrast == "Chinstrap - Gentoo"])`) Pinguine, welche ein durchschnittliches Körpergewicht von rund `r round(CIs$response[CIs$species == "Adelie"], 0)` g und `r round(CIs$response[CIs$species == "Chinstrap"], 0)` g aufweisen. Adelie und Chinstrap unterscheiden sich nicht signifikant (p = `r round(CIp$p.value[CIp$contrast == "Chinstrap - Adelie"], 2)`).
