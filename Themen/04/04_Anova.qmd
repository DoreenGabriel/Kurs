...
-   und für E `r round(coef(mod)[1],1)` + `r round(coef(mod)[4],1)` = `r round(coef(mod)[1]+coef(mod)[4],1)`
...
Hier wird immer gegen die Qualität A gepürft, weil diese das erste Level der Variable `Quali`ist.
