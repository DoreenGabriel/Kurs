...
Zunächst müssen wir die `library(ggplot2)` laden.
...
show_col(colors()[101:200], ncol=10)
show_col(colors()[201:300], ncol=10)
show_col(colors()[301:400], ncol=10)
show_col(colors()[401:500], ncol=10)
show_col(colors()[501:600], ncol=10)
show_col(colors()[601:657], ncol=10)
```
...
Wir plotten die Beziehung zwischen Infektion und  Luftfeuchtigkeit (`hum = humidity`). 

Beispiele mit unterschiedlichen Links (logit, probit und cloglog)
```{r}
#logit
ggplot(data=pot, aes(x=hum, y=infection)) +
  geom_point()+
...
## Übung 3.1.
Lesen Sie die Daten Kursteilnehmer_2021.xlsx in R ein und benennen Sie Ihren data.frame mit md.

Lernen Sie folgende Funktionen kennen:  	
...
+ den Zusammenhang zwischen Körpergröße und Schuhgröße mit 
    + unterschiedlichen Farben für die Frauen und Männer und fügen Sie Regressionslinien ein  
    
Zusatzaufgabe:  

+ verwenden Sie statt geom_point() die Funktion geom_count()
+ finden Sie heraus, was mit dem Argument `scales` der Funktion `facet_wrap()` geändert werden kann
...
### jittered boxplot für weight ~ method und weight 
...
### Transformation der Achsenabstände
...
+ und labeln die Skala bei `y+1` mit den Werten von `y` 
...
Mit der Funktion `theme()´ können Gitterlinien, Beschriftungen, Legenden und vieles mehr angepasst werden. 
...
## Alternative zum Boxplot: Dotplot
...
+ Plotten Sie einen jittered Boxplot von Körpergröße in Relation zum Geschlecht
...
### Vier Plots in einer Graphik mit einer Legende mit `library(ggpubr)` 
...
### Beschriftung innerhalb `facet_grid()` und `facet_wrap()`
...
+ Plotten Sie einen Mosaikplot für das Auftreten der Kombinationen aus Augenfarbe und Geschlecht  
